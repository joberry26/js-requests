...
sayHelloButton.addEventListener('mouseover', chgColor);
...
function chgColor2 (event){
    sayHelloButton.style.backgroundColor = '#EFEFEF';
    sayHelloButton.style.color= 'black';
}

sayHelloButton.addEventListener('mouseout', chgColor2);
...
const ohMy = () => {
    ...
    .then((res) => {
        for (i = 0; i < res.data.length; i++) {
            res.data[i].textContent = document.createElement("p")
            // document.querySelectorAll('p').textContent(res.data[i])
           console.log(res.data[i])
           
        }
        document.querySelector("body").append(res.data)
    })
}
...
const repeatMyParam = () => {
    ...
    .then((res) => {
        ...
        console.log(document.querySelector('#repeat-text').textContent = res.data)
    })
    .catch((err) => {
        console.log(err)
    })
...
function getRqst () {
axios
    .get('http://localhost:3000/query-test', {params: {goose: "bone"}} )
    .then((res) => {
        console.log(res)
    })
}

document.querySelector('#query-button').addEventListener('click', getRqst)
////////////////
//INTERMEDIATE//
////////////////
...
/* 
    ...
    On each iteration of the loop, create a new p element. Set its textContent equal the
     string at the current index (i) and then append the new p element onto the document's 
     body. 
*/
...
/*
   ...
   my console.log sent back {data: 'You sent query: goose with value: bone!',
...
/*
    ...
    */
    function createFood () {
        ...
        axios
            ...
            .then((res) => {
                ...
                document.querySelector("form").append(res.data)
            })
    ...
    one key-value pair. The key should be newFood (make sure to match the case 
    ...
    callback function to the .then. Inside that callback, console log the res.data. 
